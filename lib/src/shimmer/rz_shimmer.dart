...
///
/// * [RzShimmerDirection.ltr] left to right direction
/// * [RzShimmerDirection.rtl] right to left direction
/// * [RzShimmerDirection.ttb] top to bottom direction
/// * [RzShimmerDirection.btt] bottom to top direction
///
enum RzShimmerDirection { ltr, rtl, ttb, btt }
...
///
@immutable
class RzShimmer extends StatefulWidget {
  final Widget child;
  final Duration period;
  final RzShimmerDirection direction;
  ...
  const RzShimmer({
    ...
    this.direction = RzShimmerDirection.ltr,
  ...
  @override
  _RzShimmerState createState() => _RzShimmerState();
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(DiagnosticsProperty<Gradient>('gradient', gradient,
        defaultValue: null));
    properties.add(EnumProperty<RzShimmerDirection>('direction', direction));
    properties.add(
...
class _RzShimmerState extends State<RzShimmer> with SingleTickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(RzShimmer oldWidget) {
    if (widget.enabled) {
      _controller.forward();
    } else {
      _controller.stop();
...
@immutable
class _RzShimmer extends SingleChildRenderObjectWidget {
  ...
  @override
  _RzShimmerFilter createRenderObject(BuildContext context) {
    return _RzShimmerFilter(percent, direction, gradient);
  }

  @override
  void updateRenderObject(BuildContext context, _RzShimmerFilter shimmer) {
...
class _RzShimmerFilter extends RenderProxyBox {
  RzShimmerDirection _direction;
  Gradient _gradient;
  double _percent;

  _RzShimmerFilter(this._percent, this._direction, this._gradient);
  ...
  set direction(RzShimmerDirection newDirection) {
  ...
  void paint(PaintingContext context, Offset offset) {
    if (child != null) {
      ...
      Rect rect;
      double dx, dy;
      if (_direction == RzShimmerDirection.rtl) {
        dx = _offset(width, -width, _percent);
        dy = 0.0;
        rect = Rect.fromLTWH(dx - width, dy, 3 * width, height);
      } else if (_direction == RzShimmerDirection.ttb) {
        dx = 0.0;
        dy = _offset(-height, height, _percent);
        rect = Rect.fromLTWH(dx, dy - height, width, 3 * height);
      } else if (_direction == RzShimmerDirection.btt) {
        dx = 0.0;
        dy = _offset(height, -height, _percent);
        rect = Rect.fromLTWH(dx, dy - height, width, 3 * height);
      } else {
